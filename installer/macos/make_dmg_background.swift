...
/// Finder layout (window points) — must match build_dmg.sh AppleScript positions
...
let brandBlue = rgb(0x1E4D8C)
let brandBlueDark = rgb(0x163A6B)
let brandBlueLight = rgb(0x2F6BB5)
...
let gradient = NSGradient(colors: [rgb(0xF4F7FA), rgb(0xFFFFFF), rgb(0xEDF2F7)])
...
text("EastmarkHK CRM", size: 22, weight: .bold, color: rgb(0x0F172A),
     x: leftX, y: 72, w: 420, h: 28)
text("Companies · Contacts · Deals · Tasks", size: 13, weight: .medium, color: rgb(0x64748B),
     x: leftX, y: 98, w: 460, h: 18)
text("Drag the app into the Applications folder", size: 12, weight: .semibold, color: brandBlueDark,
     x: leftX, y: 118, w: 420, h: 16)

// CRM preview card — right of description
...
cardPath.fill()
brandBlue.withAlphaComponent(0.25).setStroke()
cardPath.lineWidth = 1.5
cardPath.stroke()

text("Open pipeline", size: 11, weight: .semibold, color: rgb(0x64748B),
     x: cardX + 12, y: cardTop + 8, w: 180, h: 14)
text("12 deals", size: 22, weight: .bold, color: brandBlue,
     x: cardX + 12, y: cardTop + 24, w: 180, h: 26)
text("Today · Follow-ups", size: 9, weight: .medium, color: rgb(0x94A3B8),
     x: cardX + 12, y: cardTop + 54, w: 160, h: 12)

let badge = rTop(cardX + cardW - 58, cardTop + 10, 46, 22)
let badgePath = NSBezierPath(roundedRect: badge, xRadius: 6, yRadius: 6)
brandBlue.withAlphaComponent(0.15).setFill()
badgePath.fill()
text("CRM", size: 10, weight: .bold, color: brandBlueDark,
     x: cardX + cardW - 54, y: cardTop + 14, w: 38, h: 14)

// Diagonal drag arrow (card → Applications)
...
func drawDiagonalArrow(from sx: CGFloat, _ sy: CGFloat, to ex: CGFloat, _ ey: CGFloat) {
    ...
    let tail = NSBezierPath(ovalIn: NSRect(x: p0.x - 9, y: p0.y - 9, width: 18, height: 18))
    rgb(0xDC2626, alpha: 0.15).setFill()
    tail.fill()
    rgb(0xDC2626).setStroke()
    tail.lineWidth = 2.5
    tail.stroke()
...
// Secondary horizontal cue between icons
...
brandBlueLight.withAlphaComponent(0.45).setStroke()
mid.stroke()
